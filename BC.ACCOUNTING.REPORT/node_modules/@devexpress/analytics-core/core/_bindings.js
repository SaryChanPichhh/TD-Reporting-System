...
const draggableInitializedPropertyName = 'draggableInitializedPropertyName';
function initUpdateDraggability(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
    const values = valueAccessor();
    if (!values)
        return;
    ...
    element.dataset[draggableInitializedPropertyName] = true;
    addDisposeCallback(element, () => {
        draggable.dispose();
        element = null;
    });
...
ko.bindingHandlers['draggable'] = {
    init: initUpdateDraggability,
    update: (element, ...rest) => {
        if (!element.dataset[draggableInitializedPropertyName]) {
            initUpdateDraggability(element, ...rest);
        }
    }
};
