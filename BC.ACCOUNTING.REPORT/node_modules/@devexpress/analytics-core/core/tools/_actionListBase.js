...
export class ActionListsBase extends BaseRenderingModel {
    ...
    processShortcut(e) {
        ...
        const isMac = devices.real()['mac'];
        const processAction = (action) => {
            e.preventDefault();
            action.clickAction();
        };
        const processHotKey = (action, hotKeyIndex) => {
            const hotKey = hotKeyIndex !== undefined ? action.hotKey[hotKeyIndex] : action.hotKey;
            hotKey.altKey ?? (hotKey.altKey = false);
            hotKey.ctrlKey ?? (hotKey.ctrlKey = false);
            const matchAlt = hotKey.altKey === e.altKey;
            const matchCtrl = (isMac && hotKey.ctrlKey === e.metaKey) || (!isMac && hotKey.ctrlKey === e.ctrlKey);
            if (hotKey.keyCode === e.keyCode && matchAlt && matchCtrl) {
                processAction(action);
            }
        };
        const processHotKeys = (action) => {
            action.hotKey.forEach((hotKey, i) => {
                processHotKey(action, i);
            });
        };
        for (let i = 0; i < actions.length; i++) {
            ...
            if (action.hotKey && enabled && visible) {
                Array.isArray(action.hotKey) ? processHotKeys(action) : processHotKey(action);
            }
        }
    }
