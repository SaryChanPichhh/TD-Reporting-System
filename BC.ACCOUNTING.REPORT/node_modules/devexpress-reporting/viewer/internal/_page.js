...
import { $dx, DxDeferred, formatUnicorn, getLocalization, toQueryParameters, base64DecodeUnicode } from '@devexpress/analytics-core/analytics-internal-native';
...
import { HandlerUri, PageRenderFormat, PreviewRenderFormat } from '../settings';
...
export class PreviewPage extends BaseRenderingModel {
    ...
    onPropertyChanged(args) {
        ...
        if (args.propertyName === 'isClientVisible') {
            if (args.newValue) {
                this._setPageImgSrc(this.reportPreview.documentId, this.reportPreview._unifier, this.zoom);
            }
        }
    ...
    _onPageLoaded(result, processClick) {
        ...
        if (!this.useSvg) {
            ...
            this.svgMarkup = base64DecodeUnicode(result.base64string);
        }
        if (this._isDisposed)
            return;
    ...
    getBricksTextContent(onlySelectedBricks = false) {
        if (this.shouldSendRequest)
            this._requestPage(this.reportPreview);
        if (onlySelectedBricks) {
            return this.reportPreview.getSelectedContent('');
        }
        else {
            return this.reportPreview.getSelectedContent('', this.bricks);
        }
    }
    activateBrick(brick) {
        if (brick.active)
    ...
    _shouldUseSvgEngine(zoom) {
        switch (PreviewRenderFormat()) {
            case PageRenderFormat.Auto:
                return this.reportPreview.showMultipagePreview ? zoom > 1 : true;
            case PageRenderFormat.Png:
                return false;
            case PageRenderFormat.Svg:
                return true;
        }
    }
    _getImageOptions() {
