...
*/
import { $dx, extend, initGlobalize, koUtils, processErrorEvent, RequestHelper, resolveFromPromises, troubleshootingPageWrapper, _addLicenseTrigger } from '@devexpress/analytics-core/analytics-internal-native';
...
import { ParametersPanelStandalone } from '../parameters/parametersPanelStandalone';
import { AIPanelViewModel } from '../ai/_aiPanelModel';
export function createStandaloneParametersPanel(bindingSettings) {
...
function _createPreview(bindingSettings) {
    ...
    if (bindingModel._licenseOptions && !bindingModel._licenseOptions.isLicensed) {
        _addLicenseTrigger(bindingModel._licenseOptions);
    }
    if (bindingModel.remoteSettings && (bindingModel.remoteSettings.authToken || bindingModel.remoteSettings.serverUri)) {
        ...
        if (bindingModel.remoteSettings.authToken) {
            ...
            if (_isFetchConfigured()) {
                fetchSetup.fetchSettings.headers = { ...fetchSetup.fetchSettings.headers, ...headers };
            }
            else {
                ajaxSetup.ajaxSettings.headers = { ...ajaxSetup.ajaxSettings.headers, ...headers };
            }
        }
    }
    let designerModel;
    ...
    if (bindingModel.requestContentType)
        requestManager._setRequestContentType(bindingModel.requestContentType.toString());
    if (bindingSettings.element && bindingSettings.applyBindings && !designerModel.reportPreview.canSwitchToDesigner) {
        ...
        $dx(bindingSettings.element).empty();
