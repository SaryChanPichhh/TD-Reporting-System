...
import { PrepareRequestArgs, RequestContentType, requestManager } from '../internal/requestManager';
...
export function _ajax(uri, action, arg, processErrorCallback, ignoreError, customOptions, isError = (data) => !data.success, getErrorMessage = igetErrorMessage, method = 'POST') {
    ...
    if (action !== undefined && arg !== undefined) {
        if (!(typeof arg === 'string') || requestManager.getRequestContentType() === RequestContentType.FormUrlEncoded)
            arg = PrepareRequestArgs(arg, true, false);
        requestData = {
            actionKey: action,
            arg: arg,
            dxversions: PrepareRequestArgs(dxversions)
        };
    }
