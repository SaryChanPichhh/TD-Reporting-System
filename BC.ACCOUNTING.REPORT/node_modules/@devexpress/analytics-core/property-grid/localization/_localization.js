...
export const Globalize = typeof window !== 'undefined' ? window['Globalize'] : undefined;
...
export function resolveFromPromises(promises, createModel) {
    ...
    if (promises.length > 0) {
        DxDeferred.when(promises).done((messages) => {
            messages.forEach(x => loadMessages(x));
...
export function formatTime(val) {
    return localization.formatDate(val, 'shortTime');
}
