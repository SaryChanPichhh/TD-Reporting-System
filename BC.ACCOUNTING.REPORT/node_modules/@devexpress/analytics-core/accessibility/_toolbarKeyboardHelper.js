...
export class ToolbarKeyboardHelper extends KeyboardHelperWithArrowButtonBase {
    ...
    handleEscKey(e) {
        const controlElement = this.controlElements.find(elt => elt.element.contains(e.target));
        controlElement?.cancelAction();
        return true;
    }
    get buttonModels() {
...
class ToolbarItemElement extends ControlElementWithParentHighlight {
    ...
    setFocus() {
        ...
        if (this._complexItem() && this.element.contains(document.activeElement))
            return;
        return super.setFocus();
    }
    actionExecute() {
        if (this._complexItem()) {
            this._liveRegion().changeText(getLocalization('Press Alt ↓ or Alt  ↑ to navigate the editor. Press Esc to exit navigation mode.', 'ASPxReportsStringId.WebDocumentViewer_AriaEditorKeyboardNavigation'), 500);
        }
        if (this._selectBox) {
